OVN-Kubernetes has many sources of built-in health checks and logs. Follow the instructions in these sections to examine your cluster. If a support case is necessary, follow the [support guide](../../support/gathering-cluster-data.xml#support_gathering_data_gathering-cluster-data) to collect additional information through a `must-gather`. Only use the `-- gather_network_logs` when instructed by support.
...
- [Implementation of connection health checks](../../networking/advanced_networking/verifying-connectivity-endpoint.xml#nw-pod-network-connectivity-implementation_verifying-connectivity-endpoint)

- [Verifying network connectivity for an endpoint](../../networking/advanced_networking/verifying-connectivity-endpoint.xml#nw-pod-network-connectivity-verify_verifying-connectivity-endpoint)
