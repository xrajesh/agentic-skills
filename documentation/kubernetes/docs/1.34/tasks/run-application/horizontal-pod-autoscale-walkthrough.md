...
the [kubelets](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.") in your cluster, and exposes those metrics
...
If you are running [Minikube](/docs/tasks/tools/#minikube "A tool for running Kubernetes locally."), run the following command to enable metrics-server:
...
`hpa-example` image, and expose it as a [Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.")
using the following manifest:

[`application/php-apache.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/application/php-apache.yaml)![](/images/copycode.svg "Copy application/php-apache.yaml to clipboard")
...
Roughly speaking, the HPA [controller](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.") will increase and decrease
...
[quantity](/docs/reference/glossary/?all=true#term-quantity "A whole-number representation of small or large numbers using SI suffixes."). For example,
...
[`application/hpa/php-apache.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/application/hpa/php-apache.yaml)![](/images/copycode.svg "Copy application/hpa/php-apache.yaml to clipboard")
