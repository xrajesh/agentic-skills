...
* Your [Pod](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.") should already be
...
[`application/nginx-with-request.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/application/nginx-with-request.yaml)![](/images/copycode.svg "Copy application/nginx-with-request.yaml to clipboard")
...
If the [container image](/docs/reference/glossary/?all=true#term-image "Stored instance of a container that holds a set of software needed to run an application.") includes
...
> **Note:**
> The `--target` parameter must be supported by the [Container Runtime](/docs/setup/production-environment/container-runtimes "The container runtime is the software that is responsible for running containers."). When not supported,
