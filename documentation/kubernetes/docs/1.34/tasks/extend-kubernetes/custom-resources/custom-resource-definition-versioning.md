...
1. Ensure all clients are fully migrated to the new version. The kube-apiserver
   ...
   the old version.
2. Set `served` to `false` for the old version in the `spec.versions` list. If
...
3. Ensure the [upgrade of existing objects to the new stored version](#upgrade-existing-objects-to-a-new-stored-version) step has been completed.
