...
resources to the [Kubelet](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.").

Instead of customizing the code for Kubernetes itself, vendors can implement a
device plugin that you deploy either manually or as a [DaemonSet](/docs/concepts/workloads/controllers/daemonset "Ensures a copy of a Pod is running across a set of nodes in a cluster.").
...
FEATURE STATE:
`Kubernetes v1.31 [alpha]`(disabled by default)
...
must be mounted as a [Volume](/docs/concepts/storage/volumes/ "A directory containing data, accessible to the containers in a pod.")
in the plugin's [PodSpec](/docs/reference/generated/kubernetes-api/v1.34/#podspec-v1-core).
...
[Volume](/docs/concepts/storage/volumes/ "A directory containing data, accessible to the containers in a pod.") in the device monitoring agent's
