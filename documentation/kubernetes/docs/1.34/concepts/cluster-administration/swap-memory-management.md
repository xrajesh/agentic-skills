...
Kubernetes can be configured to use swap memory on a [node](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes."),
...
Pods in the Guaranteed [QoS class](/docs/concepts/workloads/pods/pod-qos/ "QoS Class (Quality of Service Class) provides a way for Kubernetes to classify pods within the cluster into several classes and make decisions about scheduling and eviction."),
...
Kubernetes 1.34 does not support allocating Pods to nodes in a way that accounts
