...
The [kubelet](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.") monitors resources
...
The kubelet does not respect your configured [PodDisruptionBudget](/docs/reference/glossary/?all=true#term-pod-disruption-budget "An object that limits the number of Pods of a replicated application that are down simultaneously from voluntary disruptions.")
or the pod's
...
If the pods are managed by a [workload](/docs/concepts/workloads/ "A workload is an application running on Kubernetes.")
management object (such as [StatefulSet](/docs/concepts/workloads/controllers/statefulset/ "A StatefulSet manages deployment and scaling of a set of Pods, with durable storage and persistent identifiers for each Pod.")
or [Deployment](/docs/concepts/workloads/controllers/deployment/ "Manages a replicated application on your cluster.")) that
...
2. `imagefs`: An optional filesystem that container runtimes can use to store
   container images (which are the read-only layers) and container writable
   layers.
3. `containerfs`: An optional filesystem that container runtime can use to
...
> `containerfs`, the Kubernetes release v1.34 requires the
...
2. [Pod Priority](/docs/concepts/scheduling-eviction/pod-priority-preemption/)
...
1. `BestEffort` or `Burstable` pods where the usage exceeds requests. These pods
   ...
   exceeds the request.
2. `Guaranteed` pods and `Burstable` pods where the usage is less than requests
...
> **Note:**
> As of Kubernetes v1.34, you cannot set a custom value
...
> `system-node-critical` [Priority](/docs/concepts/scheduling-eviction/pod-priority-preemption/#pod-priority "Pod Priority indicates the importance of a Pod relative to other Pods.").
...
* Check out the [Eviction API](/docs/reference/generated/kubernetes-api/v1.34/#create-eviction-pod-v1-core)
