...
Once a Pod is scheduled to a node, the kubelet on that node creates a new [cgroup](/docs/reference/glossary/?all=true#term-cgroup "A group of Linux processes with optional resource isolation, accounting and limits.") for the Pod. It is within this pod that the underlying
