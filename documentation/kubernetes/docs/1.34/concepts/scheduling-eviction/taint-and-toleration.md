...
is a property of [Pods](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.") that *attracts* them to
a set of [nodes](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes.") (either as a preference or a
...
[`pods/pod-with-toleration.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/pods/pod-with-toleration.yaml)![](/images/copycode.svg "Copy pods/pod-with-toleration.yaml to clipboard")
...
The control plane, using the node [controller](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state."),
...
toleration on pods that have a [QoS class](/docs/concepts/workloads/pods/pod-qos/ "QoS Class (Quality of Service Class) provides a way for Kubernetes to classify pods within the cluster into several classes and make decisions about scheduling and eviction.")
