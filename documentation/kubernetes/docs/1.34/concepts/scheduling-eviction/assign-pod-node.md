...
You can constrain a [Pod](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.") so that it is
*restricted* to run on particular [node(s)](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes."),
...
[scheduler](/docs/reference/command-line-tools-reference/kube-scheduler/ "Control plane component that watches for newly created pods with no assigned node, and selects a node for them to run on.") will automatically do a reasonable placement
...
If you use labels for node isolation, choose label keys that the [kubelet](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.")
...
1. Ensure you are using the [Node authorizer](/docs/reference/access-authn-authz/node/) and have *enabled* the `NodeRestriction` admission plugin.
...
[`pods/pod-with-node-affinity.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/pods/pod-with-node-affinity.yaml)![](/images/copycode.svg "Copy pods/pod-with-node-affinity.yaml to clipboard")
...
[`pods/pod-with-affinity-preferred-weight.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/pods/pod-with-affinity-preferred-weight.yaml)![](/images/copycode.svg "Copy pods/pod-with-affinity-preferred-weight.yaml to clipboard")
...
[`pods/pod-with-pod-affinity.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/pods/pod-with-pod-affinity.yaml)![](/images/copycode.svg "Copy pods/pod-with-pod-affinity.yaml to clipboard")
...
You can use *topology spread constraints* to control how [Pods](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.")
