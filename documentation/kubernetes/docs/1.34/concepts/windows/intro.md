...
[Pods](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.") on Kubernetes is similar to
scheduling Linux-based containers.

In order to run Windows containers, your Kubernetes cluster must include
multiple operating systems.
While you can only run the [control plane](/docs/reference/glossary/?all=true#term-control-plane "The container orchestration layer that exposes the API and interfaces to define, deploy, and manage the lifecycle of containers.") on Linux,
you can deploy worker nodes running either Windows or Linux.

Windows [nodes](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes.") are
...
* [Services](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.")
...
* [Horizontal pod autoscaling](/docs/tasks/run-application/horizontal-pod-autoscale/ "Object that automatically scales the number of pod replicas based on targeted resource utilization or custom metric targets.")
* [Resource quotas](/docs/concepts/policy/resource-quotas/ "Provides constraints that limit aggregate resource consumption per namespace.")
...
Kubernetes v1.34 does **not** include the `WindowsHostNetwork` feature gate
...
For Kubernetes v1.34.0 the recommended pause image is `registry.k8s.io/pause:3.6`.
...
You can use [ContainerD](https://containerd.io/docs/ "A container runtime with an emphasis on simplicity, robustness and portability") 1.4.0+
...
For Kubernetes v1.34, operating system compatibility for Windows nodes (and Pods)
is as follows:
