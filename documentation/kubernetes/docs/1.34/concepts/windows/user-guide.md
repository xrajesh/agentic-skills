...
Users need to use some combination of [taint](/docs/concepts/scheduling-eviction/taint-and-toleration/ "A core object consisting of three required properties: key, value, and effect. Taints prevent the scheduling of pods on nodes or node groups.")
...
All Kubernetes nodes running Kubernetes 1.34 have the following default labels:
...
The best practice for Kubernetes 1.34 is to use a `nodeSelector`.
...
[RuntimeClass](/docs/concepts/containers/runtime-class/) can be used to simplify the process of using taints and tolerations.
