...
deletion.
If a [Node](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes.") dies, the Pods running on (or scheduled
...
If a Pod is scheduled to a [node](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes.") and that
node then fails, the Pod is treated as unhealthy and Kubernetes eventually deletes the Pod.
A Pod won't survive an [eviction](/docs/concepts/scheduling-eviction/ "Process of terminating one or more Pods on Nodes") due to
a lack of resources or Node maintenance.

Kubernetes uses a higher-level abstraction, called a
[controller](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state."), that handles the work of
...
[PodStatus](/docs/reference/generated/kubernetes-api/v1.34/#podstatus-v1-core)
...
Once the [scheduler](/docs/reference/command-line-tools-reference/kube-scheduler/ "Control plane component that watches for newly created pods with no assigned node, and selects a node for them to run on.")
assigns a Pod to a Node, the kubelet starts creating containers for that Pod
using a [container runtime](/docs/setup/production-environment/container-runtimes "The container runtime is the software that is responsible for running containers.").
...
image registry, or applying [Secret](/docs/concepts/configuration/secret/ "Stores sensitive information, such as passwords, OAuth tokens, and ssh keys.")
data.
...
[init containers](/docs/concepts/workloads/pods/init-containers/ "One or more initialization containers that must run to completion before any app containers run.") run only once,
during Pod startup.
...
The `restartPolicy` for a Pod applies to [app containers](/docs/reference/glossary/?all=true#term-app-container "A container used to run part of a workload. Compare with init container.")
...
restart policy. Container restart policy and rules applies to [app containers](/docs/reference/glossary/?all=true#term-app-container "A container used to run part of a workload. Compare with init container.")
in the Pod and to regular [init containers](/docs/concepts/workloads/pods/init-containers/).
...
[PodConditions](/docs/reference/generated/kubernetes-api/v1.34/#podcondition-v1-core)
...
[operators](/docs/concepts/extend-kubernetes/operator/ "A specialized controller used to manage a custom resource") should use
the `PATCH` action.
...
a container runtime (using [Container Runtime Interface (CRI)](/docs/concepts/architecture/cri "Protocol for communication between the kubelet and the local container runtime.")) to set up a
...
A *probe* is a diagnostic performed periodically by the [kubelet](/docs/reference/command-line-tools-reference/kubelet/)
...
:   Indicates whether the container is ready to respond to requests.
    If the readiness probe fails, the [EndpointSlice](/docs/concepts/services-networking/endpoint-slices/ "EndpointSlices track the IP addresses of Pods for Services.")
...
place, the [kubelet](/docs/reference/command-line-tools-reference/kubelet "An agent that runs on each node in the cluster. It makes sure that containers are running in a pod.") attempts graceful
shutdown.
...
[API Server](/docs/concepts/architecture/#kube-apiserver "Control plane component that serves the Kubernetes API."). If the kubelet or the
...
2. The Pod in the API server is updated with the time beyond which the Pod is considered "dead"
   ...
   2. The kubelet triggers the container runtime to send a TERM signal to process 1 inside each
      ...
      [sidecar containers](/docs/concepts/workloads/pods/sidecar-containers/ "An auxilliary container that stays running throughout the lifecycle of a Pod.") defined.
...
3. At the same time as the kubelet is starting graceful shutdown of the Pod, the control plane
   ...
   where those objects represent a [Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.")
   with a configured [selector](/docs/concepts/overview/working-with-objects/labels/ "Allows users to filter a list of resources based on labels.").
   [ReplicaSets](/docs/concepts/workloads/controllers/replicaset/ "ReplicaSet ensures that a specified number of Pod replicas are running at one time") and other workload resources
...
4. The kubelet ensures the Pod is shut down and terminated
   ...
   3. The kubelet triggers forcible removal of the Pod object from the API server, by setting grace period
...
[controller](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.") process
