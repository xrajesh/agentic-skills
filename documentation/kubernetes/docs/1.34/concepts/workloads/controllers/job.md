...
[`controllers/job.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/controllers/job.yaml)![](/images/copycode.svg "Copy controllers/job.yaml to clipboard")
...
It has exactly the same schema as a [Pod](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster."),
...
* If the Job [Controller](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.") has not had time to react.
...
* `Indexed`: the Pods of a Job get an associated completion index from 0 to
  ...
  + As part of the Pod hostname, following the pattern `$(job-name)-$(index)`.
    ...
    [Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service."), Pods within the Job can use
...
[`/controllers/job-backoff-limit-per-index-example.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples//controllers/job-backoff-limit-per-index-example.yaml)![](/images/copycode.svg "Copy /controllers/job-backoff-limit-per-index-example.yaml to clipboard")
...
* To guarantee that your Job finishes even if there are disruptions, you can
  ignore Pod failures caused by disruptions (such as [preemption](/docs/concepts/scheduling-eviction/pod-priority-preemption/#preemption "Preemption logic in Kubernetes helps a pending Pod to find a suitable Node by evicting low priority Pods existing on that Node."),
  ...
  or [taint](/docs/concepts/scheduling-eviction/taint-and-toleration/ "A core object consisting of three required properties: key, value, and effect. Taints prevent the scheduling of pods on nodes or node groups.")-based eviction) so
...
[`/controllers/job-pod-failure-policy-example.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples//controllers/job-pod-failure-policy-example.yaml)![](/images/copycode.svg "Copy /controllers/job-pod-failure-policy-example.yaml to clipboard")
...
[`/controllers/job-success-policy.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples//controllers/job-success-policy.yaml)![](/images/copycode.svg "Copy /controllers/job-success-policy.yaml to clipboard")
...
> Even though the [control plane](/docs/reference/glossary/?all=true#term-control-plane "The container orchestration layer that exposes the API and interfaces to define, deploy, and manage the lifecycle of containers.") eventually
