...
A Kubernetes [Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.") is an abstraction
...
| Direct Server Return (DSR) | See [DSR](#dsr) notes above. | Windows Server 2019 | Set the following command line argument (assuming version 1.34):  `--enable-dsr=true` |
