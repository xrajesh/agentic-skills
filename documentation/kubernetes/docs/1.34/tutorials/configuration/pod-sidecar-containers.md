...
that some [addons](/docs/concepts/cluster-administration/addons/ "Resources that extend the functionality of Kubernetes.") modify Pods - before the Pods
...
1. You can configure a native sidecar container to start ahead of
   [init containers](/docs/concepts/workloads/pods/init-containers/ "One or more initialization containers that must run to completion before any app containers run.").
...
3. Develop a universal sidecar injector. The idea of a universal sidecar injector is to
