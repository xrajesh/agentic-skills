...
* **API group** - The [API Group](/docs/concepts/overview/kubernetes-api/#api-groups-and-versioning "A set of related paths in the Kubernetes API.") being accessed (for resource requests only). An empty string designates the *core* [API group](/docs/reference/using-api/#api-groups).
...
and their associated [controllers](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.").
