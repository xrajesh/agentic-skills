...
[Return to the regular view of this page](/docs/reference/labels-annotations-taints/).

# Well-Known Labels, Annotations and Taints

* 1: [Audit Annotations](#pg-0adf63217075ed2dc0a3030d9673e75e)
...
The Kubernetes API server (part of the [control plane](/docs/reference/glossary/?all=true#term-control-plane "The container orchestration layer that exposes the API and interfaces to define, deploy, and manage the lifecycle of containers."))
...
For a static Pod created by the kubelet on a node, a [mirror Pod](/docs/reference/glossary/?all=true#term-mirror-pod "An object in the API server that tracks a static pod on a kubelet.")
...
[Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service."). This annotation has no effect in
Kubernetes 1.34.
...
2. Secrets referenced in `envFrom` for containers (including sidecar containers and init containers)
...
3. Secrets referenced in a Pod's `imagePullSecrets` must be present in the
...
was installed by the [controller](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.").
...
The [control plane](/docs/reference/glossary/?all=true#term-control-plane "The container orchestration layer that exposes the API and interfaces to define, deploy, and manage the lifecycle of containers.") adds
this [label](/docs/concepts/overview/working-with-objects/labels "Tags objects with identifying attributes that are meaningful and relevant to users.") to EndpointSlice and
Endpoints objects when the owning [Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.") is
...
This label records the [name](/docs/concepts/overview/working-with-objects/names "A client-provided string that refers to an object in a resource URL, such as /api/v1/pods/some-name.") of the
...
This annotation records the [name](/docs/concepts/overview/working-with-objects/names "A client-provided string that refers to an object in a resource URL, such as /api/v1/pods/some-name.") of the
...
This annotation records the [unique ID](/docs/concepts/overview/working-with-objects/names "A Kubernetes systems-generated string to uniquely identify objects.") of the
ServiceAccount that the token (stored in the Secret of type `kubernetes.io/service-account-token`)
represents.
...
[selectors](/docs/concepts/overview/working-with-objects/labels/ "Allows users to filter a list of resources based on labels.").
...
The [control plane](/docs/reference/glossary/?all=true#term-control-plane "The container orchestration layer that exposes the API and interfaces to define, deploy, and manage the lifecycle of containers.") adds this annotation to
an [Endpoints](/docs/concepts/services-networking/service/#endpoints) object if the associated
[Service](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.") has more than 1000 backing endpoints.
...
The [control plane](/docs/reference/glossary/?all=true#term-control-plane "The container orchestration layer that exposes the API and interfaces to define, deploy, and manage the lifecycle of containers.") previously used
...
(which allows reconciling arbitrary RBAC objects in a [manifest](/docs/reference/glossary/?all=true#term-manifest "A serialized specification of one or more Kubernetes API objects."))
...
Kubernetes v1.34 does not support the PodSecurityPolicy API.
