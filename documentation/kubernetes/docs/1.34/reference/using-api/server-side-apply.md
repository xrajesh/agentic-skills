...
Server-Side Apply helps users and [controllers](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.")
...
For a [CustomResourceDefinition](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/ "Custom code that defines a resource to add to your Kubernetes API server without building a complete custom server."),
...
[schema](/docs/reference/generated/kubernetes-api/v1.34#jsonschemaprops-v1-apiextensions-k8s-io)
...
[`application/ssa/nginx-deployment.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/application/ssa/nginx-deployment.yaml)![](/images/copycode.svg "Copy application/ssa/nginx-deployment.yaml to clipboard")
...
[`application/ssa/nginx-deployment-no-replicas.yaml`](https://raw.githubusercontent.com/kubernetes/website/release-1.34/content/en/examples/application/ssa/nginx-deployment-no-replicas.yaml)![](/images/copycode.svg "Copy application/ssa/nginx-deployment-no-replicas.yaml to clipboard")
...
mechanism for [controllers](/docs/concepts/architecture/controller/ "A control loop that watches the shared state of the cluster through the apiserver and makes changes attempting to move the current state towards the desired state.")
...
[RBAC](/docs/reference/access-authn-authz/rbac/ "Manages authorization decisions, allowing admins to dynamically configure access policies through the Kubernetes API.")) requires the **patch** permission to
