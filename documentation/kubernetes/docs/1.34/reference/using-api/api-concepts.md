...
[name](/docs/concepts/overview/working-with-objects/names "A client-provided string that refers to an object in a resource URL, such as /api/v1/pods/some-name.") to allow idempotent creation and
...
Within a [namespace](/docs/concepts/overview/working-with-objects/namespaces "An abstraction used by Kubernetes to support isolation of groups of resources within a single cluster."), only one object
...
1. List all of the pods on a cluster in Protobuf format.
   ...
   ```
2. Create a pod by sending Protobuf encoded data to the server, but request a response
   in JSON.
...
[aggregation layer](/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/ "The aggregation layer lets you install additional Kubernetes-style APIs in your cluster.").
...
[CustomResourceDefinition](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/ "Custom code that defines a resource to add to your Kubernetes API server without building a complete custom server.") may be encoded to the
[CBOR](https://www.rfc-editor.org/rfc/rfc8949) binary data format. CBOR is also supported at the
[aggregation layer](/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/ "The aggregation layer lets you install additional Kubernetes-style APIs in your cluster.") if it is enabled in
...
FEATURE STATE:
`Kubernetes v1.16 [beta]`(enabled by default)
...
FEATURE STATE:
`Kubernetes v1.29 [stable]`(enabled by default)
...
[selectors](/docs/concepts/overview/working-with-objects/labels/ "Allows users to filter a list of resources based on labels.") then the number of
...
[aggregation layer](/docs/concepts/extend-kubernetes/api-extension/apiserver-aggregation/ "The aggregation layer lets you install additional Kubernetes-style APIs in your cluster."), and those
...
1. The field is unrecognized because it is not in the resource's OpenAPI schema. (One
   exception to this is for [CRDs](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/ "Custom code that defines a resource to add to your Kubernetes API server without building a complete custom server.")
...
Starting from v1.33, Kubernetes (including v1.34) offers a way to define field validations using *declarative tags*.
...
FEATURE STATE:
`Kubernetes v1.19 [stable]`(enabled by default)
...
[admission controllers](/docs/reference/access-authn-authz/admission-controllers/ "A piece of code that intercepts requests to the Kubernetes API server prior to persistence of the object.")
...
declare in their
[configuration object](/docs/reference/generated/kubernetes-api/v1.34/#validatingwebhook-v1-admissionregistration-k8s-io)
...
[RBAC](/docs/reference/access-authn-authz/rbac/ "Manages authorization decisions, allowing admins to dynamically configure access policies through the Kubernetes API.") that allows patching
Deployments:
...
:   Strategic Merge Patch (a Kubernetes-specific extension based on JSON).
    ...
    defined using a [CustomResourceDefinition](/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/ "Custom code that defines a resource to add to your Kubernetes API server without building a complete custom server.").
...
[v1.meta/ObjectMeta](/docs/reference/generated/kubernetes-api/v1.34/#objectmeta-v1-meta) -
The `metadata.resourceVersion` of a resource instance identifies the resource version the instance was last modified at.

[v1.meta/ListMeta](/docs/reference/generated/kubernetes-api/v1.34/#listmeta-v1-meta) -
...
:   Return data at the most recent resource version. The returned data must be
    ...
    For etcd v3.4.31+ and v3.5.13+, Kubernetes 1.34 serves “most recent” reads from the *watch cache*:
...
> [list metadata](/docs/reference/generated/kubernetes-api/v1.34/#listmeta-v1-meta)
> of the collection as well as
> [object metadata](/docs/reference/generated/kubernetes-api/v1.34/#objectmeta-v1-meta)
