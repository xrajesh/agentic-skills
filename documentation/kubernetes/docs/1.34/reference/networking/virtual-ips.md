...
Every [node](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes.") in a Kubernetes
[cluster](/docs/reference/glossary/?all=true#term-cluster "A set of worker machines, called nodes, that run containerized applications. Every cluster has at least one worker node.") runs a
...
mechanism for [Services](/docs/concepts/services-networking/service/ "A way to expose an application running on a set of Pods as a network service.")
of `type` other than
...
for the addition and removal of Service and [EndpointSlice](/docs/concepts/services-networking/endpoint-slices/ "EndpointSlices track the IP addresses of Pods for Services.")
[objects](/docs/concepts/overview/working-with-objects/#kubernetes-objects "An entity in the Kubernetes system, representing part of the state of your cluster."). For each Service, kube-proxy
...
[Pods](/docs/concepts/workloads/pods/ "A Pod represents a set of running containers in your cluster.") for a stateless
...
Service backed by a [Deployment](/docs/concepts/workloads/controllers/deployment/ "Manages a replicated application on your cluster.")
...
If you are not running kube-proxy from Kubernetes 1.34, check
...
![](/images/docs/services-ipvs-overview.svg)
...
As of Kubernetes 1.34, the `nftables` mode is
...
CIDR range that is configured for the [API Server](/docs/concepts/architecture/#kube-apiserver "Control plane component that serves the Kubernetes API.").

### IP address allocation tracking

To ensure each Service receives a unique IP address, an internal allocator atomically
updates a global allocation map in [etcd](/docs/tasks/administer-cluster/configure-upgrade-etcd/ "Consistent and highly-available key value store used as backing store of Kubernetes for all cluster data.")
