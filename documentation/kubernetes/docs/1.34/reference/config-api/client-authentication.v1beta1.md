...
| `spec` **[Required]**  [`ExecCredentialSpec`](#client-authentication-k8s-io-v1beta1-ExecCredentialSpec) | Spec holds information passed to the plugin by the transport. |
| `status`  [`ExecCredentialStatus`](#client-authentication-k8s-io-v1beta1-ExecCredentialStatus) | Status is filled in by the plugin and holds the credentials that the transport should use to contact the API. |
...
| Field | Description |
| --- | --- |
| `cluster`  [`Cluster`](#client-authentication-k8s-io-v1beta1-Cluster) | Cluster contains information to allow an exec plugin to communicate with the kubernetes cluster being authenticated to. Note that Cluster is non-nil only when provideClusterInfo is set to true in the exec provider config (i.e., ExecConfig.ProvideClusterInfo). |
