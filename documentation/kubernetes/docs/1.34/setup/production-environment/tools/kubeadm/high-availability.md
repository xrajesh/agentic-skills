...
* Three or more machines that meet [kubeadm's minimum requirements](/docs/setup/production-environment/tools/kubeadm/install-kubeadm/#before-you-begin) for
  ...
  + including a [container runtime](/docs/setup/production-environment/container-runtimes "The container runtime is the software that is responsible for running containers."), already set up and working
...
2. Add the first control plane node to the load balancer, and test the
   connection:
...
3. SSH between nodes to check that the connection is working correctly.
   ...
   * When using sudo on any node, make sure to preserve the environment so SSH
     ...
     sudo -E -s
     ```
4. After configuring SSH on all the nodes you should run the following script on the first
...
5. Then on each joining control plane node you have to run the following script before running `kubeadm join`.
