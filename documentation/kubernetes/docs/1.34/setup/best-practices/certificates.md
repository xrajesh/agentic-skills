...
* [Server certificates](/docs/reference/access-authn-authz/kubelet-tls-bootstrapping/#client-and-serving-certificates)
  for each kubelet (every [node](/docs/concepts/architecture/nodes/ "A node is a worker machine in Kubernetes.") runs a kubelet)
...
| kube-apiserver | kubernetes-ca |  | server | `<hostname>`, `<Host_IP>`, `<advertise_IP>`[1](#fn:1) |
...
1. any other IP or DNS name you contact your cluster on (as used by [kubeadm](/docs/reference/setup-tools/kubeadm/)
   ...
   `kubernetes.default.svc.cluster`, `kubernetes.default.svc.cluster.local`) [↩︎](#fnref:1)
